...
from xingcheng.localstore import LocalError, LocalStore, encode
...
def test_backup_restore_evidence_integrity_and_export_identity(local, tmp_path):
    ...
    write_download(app, wid, 1, {"alarm": 15}, tmp_path / "one.ics", profile["id"])
    app.save_template(
        "班次",
...
def test_restore_stale_preview_version_and_sequence_never_regress(local, tmp_path):
    app, wid = local
    imported(app, wid)
    write_download(app, wid, 1, {"alarm": 15}, tmp_path / "one.ics")
    backup = pack_backup(app.store, wid)
    write_download(app, wid, 1, {"alarm": 30}, tmp_path / "two.ics")
    ...
    result = write_download(app, wid, 2, {"alarm": 15}, tmp_path / "three.ics")
...
def test_template_course_and_export_filters(local, tmp_path):
    ...
    with pytest.raises(LocalError):
        write_download(app, wid, 0, {}, tmp_path / "x.ics")
    with pytest.raises(LocalError):
        write_download(app, wid, 1, {"category": "工作"}, tmp_path / "x.ics")
    change(
        app, wid, dict(type="manual", values=dict(date="2027-01-01", title="时间待填"))
    )
    assert app.export_preview(wid, {})["blocked"]
    with pytest.raises(LocalError):
        write_download(app, wid, 2, {}, tmp_path / "x.ics")


def write_download(app, wid, version, options, path, profile_id=""):
    result = app.export_file(wid, version, options, Path(path).name, profile_id)
    Path(path).write_bytes(base64.b64decode(result["download"]["data"]))
    return result
...
def finish(app, jid):
    ...
    job = app.store.document("job", jid)
    for f in job["files"]:
        if f["status"] != "queued":
            continue
        try:
            report = execute(
                dict(
                    kind="parse",
                    filename=f["filename"],
                    name=app.workspace(job["workspace_id"])["name"],
                    year=job["year"],
                    rules=job["rules"],
                    layout_hint=job["rules"].get("template"),
                ),
                app.jobs.file_data(f),
            )["report"]
            app.jobs.finish(jid, f["id"], job["generation"], report=report)
        except ValueError as e:
            app.jobs.finish(jid, f["id"], job["generation"], error=str(e))
    return app.get_job(jid)


def input_files(paths):
    return [
        dict(
            filename=Path(p).name, data=base64.b64encode(Path(p).read_bytes()).decode()
        )
        for p in paths
    ]
