...
def test_only_current_static_origin_can_call_native_bridge(local, tmp_path):
    ...
    try:
        ...
        for untrusted in ('file:///tmp/index.html', 'https://example.org', url.replace('127.0.0.1', 'localhost'), url.replace('127.0.0.1', '127.0.0.1.example.org'), url.replace('http:', 'https:')):
            assert not allowed_origin(untrusted)
            bridge = Bridge(app)
            bridge._window = SimpleNamespace(get_current_url=lambda: untrusted)
            assert bridge.call('create_workspace', {'name':'不应创建'})['error']['code'] == 'ACCESS_DENIED'
...
@pytest.mark.parametrize('platform,extension', [('win32','.exe'),('darwin',''),('linux','')])
def test_packaged_worker_stays_next_to_executable(monkeypatch, tmp_path, platform, extension):
    executable = tmp_path / ('包含 空格的目录') / ('CaliSift' + extension)
    monkeypatch.setattr(sys, 'frozen', True, raising=False)
    monkeypatch.setattr(sys, 'platform', platform)
    monkeypatch.setattr(sys, 'executable', str(executable))
    assert worker_command() == [str(executable.with_name('calisift-worker' + extension))]


def test_category_validation_and_device_preferences_do_not_change_calendar(local):
    app, wid = local
    revision = app.workspace(wid)['version']
    device = app.device_preferences({'last_workspace':wid, 'width':900, 'panel_ratio':65})
    ...
    for category in [dict(name=' 工作',color='#123456',active=True), dict(name='项目',color='red',active=True), dict(name='工作',color='#123456',active=True)]:
        with pytest.raises(ValueError):
            app.save_preferences(wid, 0, {**values,'categories':values['categories']+[category]})
    with pytest.raises(ValueError):
        Preferences.model_validate({**values,'categories':values['categories'][1:]})
    assert from_legacy({'colors':{'工作':'invalid'}})['categories'][0]['color'] == values['categories'][0]['color']
