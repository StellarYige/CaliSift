...
当前为 **0.3.0-alpha.1 Windows 桌面开发预览**。无需微信、云服务器、账号或大模型密钥。图片使用本地 RapidOCR / ONNX Runtime CPU；Excel / CSV 直接解析。

![在原文旁边核对自己的安排](docs/images/workbench.png)

## 下载与使用

Windows x64 完整安装包通过 [Releases](https://github.com/StellarYige/CaliSift/releases) 分发，文件名为 `CaliSift-0.3.0-alpha.1-windows-x64-setup.exe`；若尚无发行资产，可以按下方说明从源码运行或构建。完整包包括 Python、OCR 引擎、固定模型和 WebView2 离线安装程序。仅在缺少 WebView2 时安装该微软运行时。签名状态及验证范围见[版本说明](docs/release-0.3.0-alpha.1.md)，具体文件校验值随发行资产提供。
...
| 规则复用 | 来源班次时间、学期节次、单双周、指定周次、表头映射、模板导入导出及结构匹配建议 |
...
预览面向 Windows 11 x64。macOS / Linux 的桌面发行包和手机日历客户端尚未完成原生验收。ICS 是静态文件，重复导入、删除旧安排和提醒效果取决于目标日历，**不代表自动同步成功**。
...
数据默认存于 `%LOCALAPPDATA%\CaliSift`，可以用 `CALISIFT_DATA_DIR` 指定独立目录。一个目录同时只允许一个桌面或 CLI 实例写入。
