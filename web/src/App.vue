...
import { call, whenReady } from "./client";
...
import OfflinePane from "./components/OfflinePane.vue";
...
whenReady(() => refresh());
window.addEventListener("calisift-error", (e: Event) =>
  notify((e as CustomEvent).detail, true),
);
</script>
<template>
  <div class="app-shell" :class="{ large: workspace?.settings.large_text }">
    <aside class="sidebar">
      ...
      <div class="sidebar-bottom">
        <span class="local-dot"></span>安排保存在当前浏览器<small
          >核对、积累，安心导出。</small
        ><span class="version">{{
          capabilities?.version || "0.4.0-alpha.1"
        }}</span>
      </div>
    </aside>
    <main>
      ...
      <section v-else-if="!workspace" class="onboarding">
        ...
        <p class="notice">
          数据属于当前浏览器和站点，同一浏览器配置会共用数据。换浏览器、换站点或清理站点数据后，需要用备份恢复。无账号、无云同步。
        </p>
        <OfflinePane />
      </section>
      <template v-else
