...
import OfflinePane from "./OfflinePane.vue";
...
const recoveryPoints = ref<any[]>([]);
async function refresh() {
  jobs.value = await call("list_jobs", { workspace_id: props.workspace.id });
  recoveryPoints.value = await call("recovery_points");
}
watch(
...
<template>
  <section class="page-stack">
    <div class="page-heading">
      <div>
        <p class="eyebrow">数据留在当前浏览器，由你掌握</p>
        <h1>设置</h1>
        <p class="muted">
    ...
    <div class="two-columns">
      <section class="card">
        ...
        <p class="notice">
          数据属于当前浏览器和站点，同一浏览器配置会共用数据。换浏览器、换站点或清理站点数据后，需要用备份恢复。
        </p>
      </section>
      <section class="card">
    ...
    <OfflinePane />
    <section v-if="recoveryPoints.length" class="card">
      <h2>替换备份前的恢复点</h2>
      <p class="muted">
        保留最近两次替换前的数据。先下载所需工作区，再用“恢复 /
        迁移备份”核对导入。
      </p>
      <div v-for="point in recoveryPoints" :key="point.index">
        <p>{{ point.created_at }}</p>
        <button
          v-for="w in point.workspaces"
          :key="w.id"
          class="secondary"
          @click="
            run(() =>
              call('recovery_backup', {
                index: point.index,
                workspace_id: w.id,
              }),
            )
    ...
    <section class="card">
      ...
      <p class="muted">
        为方便恢复核对，未完成任务在浏览器中暂存原文件副本。确认导入或丢弃任务后清理。
      </p>
      <article
